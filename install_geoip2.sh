...
echo -e "${GREEN}[3/3] 部署 LibreSpeed 容器（3-5分钟）...${NC}"
create_librespeed() {
    docker run -d \
      --restart always \
      --name librespeed \
      -p 127.0.0.1:$SPEED_PORT:80 \
      adolfintel/speedtest >> "$LOG_FILE" 2>&1
    if [ $? -ne 0 ]; then
        log_error "${RED}❌ LibreSpeed 容器启动失败！${NC}"
        exit 1
    fi
    log_info "${GREEN}✅ LibreSpeed 部署成功，端口：$SPEED_PORT）${NC}"
}
CURRENT_PORT=$(docker inspect librespeed --format '{{(index (index .NetworkSettings.Ports "80/tcp") 0).HostPort}}' 2>/dev/null || echo "")
if [ -n "$CURRENT_PORT" ]; then
    if [ "$CURRENT_PORT" = "$SPEED_PORT" ]; then
        log_info "${GREEN}✅ LibreSpeed 容器已存在且端口匹配，跳过重建${NC}"
        docker start librespeed >> "$LOG_FILE" 2>&1
    else
        log_warn "${YELLOW}⚠️ 端口不匹配（当前：$CURRENT_PORT → 目标：$SPEED_PORT），自动重建...${NC}"
        docker stop librespeed >> "$LOG_FILE" 2>&1
        docker rm librespeed >> "$LOG_FILE" 2>&1
        create_librespeed
    fi
else
