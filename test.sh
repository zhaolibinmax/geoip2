...
# ===== 替换：Cloudreve 相关变量 =====
DEFAULT_CLOUDREVE_PORT=5212
CLOUDREVE_PORT=$DEFAULT_CLOUDREVE_PORT
# Cloudreve 数据卷目录（持久化数据）
CLOUDREVE_DATA="/opt/cloudreve/data"
CLOUDREVE_CONF="/opt/cloudreve/conf"
CLOUDREVE_UPLOAD="/opt/cloudreve/uploads"
# 数据库相关（Cloudreve 默认内置SQLite，无需额外配置）
...
trap 'restore_nginx_config; exit 1' INT TERM
# ==================== 函数定义 (GeoIP2 相关) ====================
test_nginx_config() {
    if ! nginx -t 2>&1; then
...
detect_package_manager() {
    if [ -f /etc/debian_version ]; then
        ...
        if grep -E "9|Stream 9" /etc/redhat-release >> "$LOG_FILE" 2>&1; then
            PM="dnf"
            PM_INSTALL="install -y"
...
install_nginx_official_repo() {
    ...
    if [ "$PM" = "apt-get" ]; then
        if [ -f /etc/debian_version ] && ! grep -s "Ubuntu" /etc/os-release; then
            log_info "${GREEN}检测到Debian系统，执行官方安装流程${NC}"
            $PM $PM_INSTALL curl gnupg2 ca-certificates lsb-release debian-archive-keyring >> "$LOG_FILE" 2>&1
            log_info "${GREEN}导入nginx官方GPG密钥...${NC}"
            curl -sSL https://nginx.org/keys/nginx_signing.key | gpg --dearmor \
...
http://nginx.org/packages/debian `lsb_release -cs` nginx" \
            ...
            log_info "${GREEN}检测到Ubuntu系统，执行官方安装流程${NC}"
            $PM $PM_INSTALL curl gnupg2 ca-certificates lsb-release ubuntu-keyring >> "$LOG_FILE" 2>&1
            log_info "${GREEN}导入nginx官方GPG密钥...${NC}"
            curl -sSL https://nginx.org/keys/nginx_signing.key | gpg --dearmor \
                | sudo tee /usr/share/keyrings/nginx-archive-keyring.gpg >/dev/null
            local expected_fingerprint="573BFD6B3D8FBC641079A6ABABF5BD827BD9BF62"
...
http://nginx.org/packages/ubuntu `lsb_release -cs` nginx" \
        ...
        log_info "${GREEN}更新apt源并安装nginx...${NC}"
        $PM $PM_UPDATE >> "$LOG_FILE" 2>&1
        $PM $PM_INSTALL nginx >> "$LOG_FILE" 2>&1
    elif [ "$PM" = "yum" ] || [ "$PM" = "dnf" ]; then
        log_info "${GREEN}检测到CentOS/RHEL系系统，执行官方安装流程${NC}"
        $PM $PM_INSTALL yum-utils >> "$LOG_FILE" 2>&1
        log_info "${GREEN}创建nginx官方yum仓库配置...${NC}"
...
EOF
        ...
        log_info "${GREEN}安装nginx（稳定版）...${NC}"
        $PM $PM_INSTALL -y nginx >> "$LOG_FILE" 2>&1
    fi
...
interactive_config() {
    clear
    echo -e "${BLUE}=====================================${NC}"
    echo -e "${BLUE}   Cloudreve + GeoIP2 一体化部署     ${NC}"
    echo -e "${BLUE}=====================================${NC}"
    echo ""
    while true; do
        ...
        if validate_domain "$INPUT_DOMAIN"; then
            ...
            log_error "${RED}错误：域名格式不合法！请输入如 cloudreve.test.com 这样的有效域名${NC}"
            echo ""
        fi
    done
    ...
    # ===== 替换：Cloudreve 端口配置 =====
    while true; do
        read -p "$(echo -e "${YELLOW}请输入 Cloudreve 服务端口 [默认: $DEFAULT_CLOUDREVE_PORT]${NC}\n端口号：")" CUSTOM_PORT
        if [ -z "$CUSTOM_PORT" ]; then
            CLOUDREVE_PORT=$DEFAULT_CLOUDREVE_PORT
            break
        fi
        if [[ "$CUSTOM_PORT" =~ ^[0-9]+$ ]] && [ "$CUSTOM_PORT" -ge 1 ] && [ "$CUSTOM_PORT" -le 65535 ]; then
            CLOUDREVE_PORT=$CUSTOM_PORT
            break
        else
    ...
    log_info "${GREEN}✅ 确认 Cloudreve 端口：$CLOUDREVE_PORT${NC}"
    echo ""
    ...
    if [ -n "$INPUT_BLOCK" ]; then
        local processed_input=$(echo "$INPUT_BLOCK" | tr -d ' ' | tr '[:upper:]' '[:upper:]')
...
echo -e "Cloudflare CDN ☁️：${GREEN}$([ "$USE_CLOUDFLARE" = true ] && echo "是" || echo "否")${NC}"
# ===== 替换：Cloudreve 端口展示 =====
echo -e "Cloudreve 端口 🚀：${GREEN}$CLOUDREVE_PORT${NC}"
...
# ==================== 步骤2: 安装 Cloudreve (Docker) ====================
log_info "${BLUE}===== 开始部署 Cloudreve 云盘服务 =====${NC}"
echo -e "${GREEN}[1/3] 正在安装 Docker 环境（3-5分钟）...${NC}"
...
echo -e "${GREEN}[2/3] 启动 Docker 并设置开机自启...${NC}"
if command -v systemctl; then
    systemctl enable docker >> "$LOG_FILE" 2>&1
    systemctl start docker
else
    if [ -f /etc/redhat-release ]; then
        chkconfig docker on >> "$LOG_FILE" 2>&1
    else
...
echo -e "${GREEN}[3/3] 部署 Cloudreve 容器（3-5分钟）...${NC}"
# ===== 替换：Cloudreve 容器部署逻辑 =====
create_cloudreve() {
    # 创建持久化目录
    mkdir -p $CLOUDREVE_DATA $CLOUDREVE_CONF $CLOUDREVE_UPLOAD
    chmod -R 755 $CLOUDREVE_DATA $CLOUDREVE_CONF $CLOUDREVE_UPLOAD
    chown -R 0:0 $CLOUDREVE_DATA $CLOUDREVE_CONF $CLOUDREVE_UPLOAD
    
    # 启动 Cloudreve 容器（内置SQLite，无需额外数据库）
    docker run -d \
      --restart always \
      --name cloudreve \
      -p 127.0.0.1:$CLOUDREVE_PORT:5212 \
    ...
    log_info "${GREEN}✅ Cloudreve 部署成功，端口：$CLOUDREVE_PORT）${NC}"
    # 输出初始管理员密码（Cloudreve 首次启动会生成）
    sleep 5
    INIT_PWD=$(docker logs cloudreve 2>&1 | grep -oP '初始管理员密码: \K.*' || echo "请手动执行 docker logs cloudreve 查看初始密码")
    log_info "${YELLOW}⚠️ Cloudreve 初始管理员密码：${GREEN}$INIT_PWD${NC}"
}
# 检查现有 Cloudreve 容器
CURRENT_PORT=$(docker inspect cloudreve --format '{{(index (index .NetworkSettings.Ports "5212/tcp") 0).HostPort}}' 2>/dev/null || echo "")
if [ -n "$CURRENT_PORT" ]; then
    if [ "$CURRENT_PORT" = "$CLOUDREVE_PORT" ]; then
        log_info "${GREEN}✅ Cloudreve 容器已存在且端口匹配，跳过重建${NC}"
        docker start cloudreve >> "$LOG_FILE" 2>&1
    else
        log_warn "${YELLOW}⚠️ 端口不匹配（当前：$CURRENT_PORT → 目标：$CLOUDREVE_PORT），自动重建...${NC}"
        docker stop cloudreve >> "$LOG_FILE" 2>&1
        docker rm cloudreve >> "$LOG_FILE" 2>&1
        create_cloudreve
    fi
else
    log_info "${YELLOW}🔒 安全提示：Cloudreve 将绑定到 127.0.0.1，仅允许本地 Nginx 反向代理访问${NC}"
    create_cloudreve
fi
# ==================== 步骤3: 部署 GeoIP2 + Cloudflare 真实IP ====================
log_info "${BLUE}===== 开始部署 GeoIP2 + Cloudflare 真实IP =====${NC}"
log_info "${GREEN}1. 检测Nginx版本...${NC}"
if ! command -v nginx; then
...
log_info "${GREEN}2. 安装系统依赖（3-5分钟）...${NC}"
$PM $PM_UPDATE >> "$LOG_FILE" 2>&1
if [ "$PM" = "apt-get" ]; then
    $PM $PM_INSTALL build-essential libpcre3-dev zlib1g-dev libmaxminddb-dev git curl libssl-dev >> "$LOG_FILE" 2>&1
else 
    $PM $PM_INSTALL gcc gcc-c++ make pcre-devel zlib-devel libmaxminddb-devel git curl openssl-devel >> "$LOG_FILE" 2>&1
    if [ "$PM" = "yum" ]; then
        $PM $PM_INSTALL epel-release -y >> "$LOG_FILE" 2>&1
    fi
fi
...
if [ ! -f "$NGINX_SRC_TAR" ] || [ ! -d "$NGINX_SRC_DIR" ] || [ ! -f "$NGINX_SRC_DIR/auto/options" ]; then
    ...
    log_info "${GREEN}正在下载 Nginx $NGINX_VERSION 源码...${NC}"
    if ! curl -sSL --connect-timeout 20 --max-time 30 --retry 3 \
        "https://nginx.org/download/${NGINX_SRC_TAR%_$$}" -o "$NGINX_SRC_TAR"; then
        log_error "${RED}❌ Nginx源码下载失败！${NC}"
        rm -rf "$NGINX_SRC_TAR"
        exit 1
    fi
    log_info "${GREEN}正在解压 Nginx 源码...${NC}"
...
log_info "${GREEN}4. 下载GeoIP2模块...${NC}"
if [ ! -d "/tmp/$MODULE_NAME" ]; then
    git clone "$MODULE_GIT_URL" /tmp/$MODULE_NAME
fi
log_info "${GREEN}✅ GeoIP2模块下载完成${NC}"
log_info "${GREEN}5. 编译GeoIP2动态模块...${NC}"
cd "$NGINX_SRC_DIR"
./configure --with-compat --add-dynamic-module=/tmp/$MODULE_NAME >> "$LOG_FILE" 2>&1 || { 
...
log_info "${GREEN}测试Nginx配置（加载模块后）...${NC}"
if ! test_nginx_config; then
    restore_nginx_config
    exit 1
fi
if [ "$USE_CLOUDFLARE" = true ]; then
    log_info "${GREEN}8. 获取 Cloudflare IP 段...${NC}"
    CF_IPV4=$(curl -sSL --fail --connect-timeout 10 --max-time 30 https://www.cloudflare.com/ips-v4 || true)
    CF_IPV6=$(curl -sSL --fail --connect-timeout 10 --max-time 30 https://www.cloudflare.com/ips-v6 || true)
...
if [ "$USE_CLOUDFLARE" = true ]; then
    log_info "${GREEN}10. 配置Cloudflare真实IP还原...${NC}"
cat > "$TEMP_CONF" << EOL
...
log_info "${GREEN}11. 检查GeoIP数据库...${NC}"
...
if [ ! -f "$DB_FILE" ]; then
    ...
    for url in "${GEOIP_DB_URLS[@]}"; do
        log_info "${GREEN}尝试从源 $url 下载...${NC}"
        curl -sSL --connect-timeout 100 --max-time 300 "$url" -o "$DB_FILE"
...
log_info "${GREEN}测试Nginx配置（GeoIP2拦截）...${NC}"
if ! test_nginx_config; then
    restore_nginx_config
    exit 1
fi
log_info "${GREEN}13. 完善站点Server配置...${NC}"
SSL_CONFIG=""
QUIC_CONFIG=""
SSL_ENABLED=false
if [ "$CUSTOM_DOMAIN" != "localhost" ]; then
...
$SSL_CONFIG
    ...
    location / {
        ...
        proxy_read_timeout 3600s;  # Cloudreve 大文件上传需要更长超时
...
SITEEOF
    log_info "${GREEN}✅ default.conf配置文件创建完成（已启用SSL，适配域名 $CUSTOM_DOMAIN 和端口 $CLOUDREVE_PORT）${NC}"
else
    cat >> "$DEFAULT_CONF" << SITEEOF
tcp_nopush on;
tcp_nodelay on;
...
SITEEOF
    log_info "${GREEN}✅ default.conf配置文件创建完成（未启用SSL，仅80端口，适配域名 $CUSTOM_DOMAIN 和端口 $CLOUDREVE_PORT）${NC}"
fi
log_info "${GREEN}测试Nginx配置（Server块）...${NC}"
if ! test_nginx_config; then
    restore_nginx_config
    exit 1
fi
log_info "${GREEN}14. 创建403错误页面...${NC}"
mkdir -p "$PAGE_ROOT"
...
mkdir -p /download && chmod 755 /download && chown root:root /download
log_info "${GREEN}15. 重启Nginx...${NC}"
...
# ==================== 完成 ====================
log_info "${BLUE}Cloudreve 容器管理命令：${NC}"
log_info "${BLUE}  查看状态：docker ps | grep cloudreve${NC}"
log_info "${BLUE}  停止服务：docker stop cloudreve${NC}"
log_info "${BLUE}  启动服务：docker start cloudreve${NC}"
log_info "${BLUE}  删除服务：docker rm -f cloudreve${NC}"
log_info "${BLUE}  查看初始密码：docker logs cloudreve | grep '初始管理员密码'${NC}"
echo -e "${BLUE}================================================${NC}"
echo -e "${GREEN}              部署完成！🎉${NC}"
echo -e "${BLUE}================================================${NC}"
echo ""
echo -e "${GREEN}部署信息汇总 📋：${NC}"
echo -e "   ${BLUE}访问地址 🌐：${NC}$([ "$SSL_ENABLED" = true ] && echo "https://$CUSTOM_DOMAIN" || echo "http://$CUSTOM_DOMAIN")"
...
echo -e "   4. Cloudreve 初始密码请执行：docker logs cloudreve | grep '初始管理员密码'"
echo -e "   5. Cloudreve 数据已持久化到 $CLOUDREVE_DATA，删除容器不会丢失数据"
echo ""
